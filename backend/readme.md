...
1. 看到节点seq=0，保存为叶子节点
2. 节点seq>现有节点seq+1，说明是未来节点，暂不保存，由于中间节点客观存在，将来一定会满足3
3. 节点seq=现有节点seq+1，替换现有节点

最终，一棵树只保留最后的叶子节点

多叉树的情况（即有冲突），最先看到的分支会替换掉公共父节点，因此其他分支需要在没有父节点的情况下还原，事实上，只要能够找出分支的第一个节点并在没有父节点的情况下保存（即将它当作根节点），就解决了问题

首先，对于一个分支，无法通过比较找出第一个节点，因为第一个节点可能由于网络原因未同步到执行还原的机器

所以只能通过制定条件区分出分支第一个节点和其他节点

制定条件如下：

**前提：每个机器只能在自己历史修改数最多的分支新增节点**

对分支第一个节点来说，看到它时，必要条件有：
- 创建该节点的机器在本节点的历史修改数至少等于同父已知分支+1
- 该节点除“创建该节点机器”历史修改数大于同父已知分支外，来自其他机器的历史修改数要小于等于同父已知分支

现证明以上条件是充分条件

对于非第一个节点：
- 创建该节点的机器在本节点的历史修改数不一定大于其他分支，该机器可能放弃了其他分支转而发展本分支
- 该节点的历史修改数中已经包含分支第一个节点创建机器的修改数，该机器修改数大于其他分支该机器修改数
...
<del>
当一个key有多个分支时，如何确定本机跟踪的是哪个分支？并且在冲突存在的情况下，持续发展该分支

1. 如果修改某分支，本机自动将跟踪该分支，并且is_main会继承下去
2. 对于来自其他机器的分支，将跟踪seq最大的分支
   1. 如何解决is_main在不同时期漂移的问题？就跟踪第一次见到的分支吧
</del>
